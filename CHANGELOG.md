...
- **Prefer rustls's built-in post-quantum key exchange without `rustls-post-quantum`.** The `post-quantum` feature now enables rustls's `prefer-post-quantum` feature on the `aws-lc-rs` provider instead of installing the separate `rustls-post-quantum` provider. This keeps `X25519MLKEM768` first in rustls's default key-exchange group order when the feature is enabled while removing the redundant dependency.
