...
- **Fix: detect the `TE: trailers` token case-insensitively (RFC 9110 §5.6.6 / §10.1.4).** Detecting whether the incoming request signalled willingness to accept trailers compared the token as raw bytes against `b"trailers"`, so any non-lowercase spelling - `Trailers`, `TRAILERS`, or `Trailers` inside a list (`gzip, Trailers`) - was silently missed, and the upstream-bound request was sent without the `te: trailers` header it should have had. RFC 9110 defines TE values as `#t-codings` and specifies transfer-coding tokens as case-insensitive; the local convention in `extract_upgrade`/`hop.rs` already uses `eq_ignore_ascii_case` for the same reason. The match now uses `<[u8]>::eq_ignore_ascii_case`, with the detection extracted into a small private free function for direct unit testing; the surrounding `.map().unwrap_or(false)` collapses to `is_some_and(...)`. Pinned by new tests covering lowercase / titlecase / uppercase, position-in-list, unrelated tokens, the empty header, and a `trailers-extra` substring safety net. No unrelated code path is changed; lowercase TE-trailers inputs keep the same upstream request bytes. Non-lowercase TE-trailers inputs now correctly emit `te: trailers` upstream, which - per RFC 9110 - may let the backend send response trailers that are forwarded downstream.
