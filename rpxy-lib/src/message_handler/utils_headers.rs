...
struct ForwardedEntry {
  /// for=:
  /// For Forwarded Header: his is optional in the RFC, but we require it for each entry to be able to build the forwarding chain. If missing, the entry is considered invalid.
  for_ip: IpAddr,
...
pub(super) fn add_forwarding_header(
  ...
  // For X-Forwarded-For
  let normalized_for_ip_chain = normalized_chain
    .iter()
    .map(|entry| entry.for_ip.to_string())
    .collect::<Vec<_>>();
...
fn normalize_forwarding_chain(
  ...
  let peer_entry = ForwardedEntry {
    for_ip: *peer_ip, // We do not append port information in for=.
    ...
    by: None, // We do not append by information
  };
...
fn reduce_trusted_proxy_chain(mut chain: Vec<ForwardedEntry>, trusted_forwarded_proxies: &[IpNet]) -> Vec<ForwardedEntry> {
  ...
  while idx > 0 && is_trusted_proxy(&chain[idx - 1].for_ip, trusted_forwarded_proxies) {
    idx -= 1;
  }
...
fn parse_forwarded_header(headers: &HeaderMap) -> Result<Vec<ForwardedEntry>> {
  let forwarded = parse_single_header_value(headers, header::FORWARDED)?.ok_or_else(|| anyhow!("forwarded header missing"))?;
...
fn parse_forwarded_header_entry(entry: &str) -> Result<ForwardedEntry> {
  ...
  let mut host = None;
  let mut by = None;
  for param in entry.split(';').map(str::trim) {
    ...
    match key.as_str() {
      "for" => {
        ...
        for_ip = Some(parse_forwarded_ip_token(value)?);
      }
      "proto" => {
        if proto.is_some() {
...
fn forwarded_is_consistent(forwarded: &[ForwardedEntry], xff_chain: &[ForwardedEntry]) -> bool {
  ...
  if !forwarded
    .iter()
    .map(|entry| entry.for_ip)
    .eq(xff_chain.iter().map(|entry| entry.for_ip))
  {
    return false;
  }
...
/// Parse single header value and ensure there is only one field-line for the given key.
fn parse_single_header_value(headers: &HeaderMap, key: impl AsHeaderName) -> Result<Option<&str>> {
  let mut values = headers.get_all(key).iter();
  let Some(first) = values.next() else {
    return Ok(None);
  };
  // Reject multiple field-lines for the same header key to prevent ambiguity and potential security issues.
  if values.next().is_some() {
    return Err(anyhow!("multiple header field-lines are not accepted"));
  }
...
fn generate_forwarded_header(normalized_forwarding_chain: &[ForwardedEntry]) -> Result<String> {
  ...
  let elements = normalized_forwarding_chain
    .iter()
    .map(|entry| {
      let mut parts = vec![format!("for={}", format_forwarded_for_value(&entry.for_ip.to_string())?)];
...
fn format_forwarded_for_value(ip: &str) -> Result<String> {
  match IpAddr::from_str(ip) {
    Ok(IpAddr::V6(v6)) => Ok(format!("\"[{}]\"", v6)),
    Ok(IpAddr::V4(v4)) => Ok(v4.to_string()),
    Err(e) => Err(anyhow!(
      "invalid x-forwarded-for address `{ip}` for Forwarded generation: {e}"
    )),
  }
}
...
mod tests {
  ...
  #[test]
  fn trusted_proxy_rejects_multiple_xff_field_lines() {
    let mut headers = HeaderMap::new();
  ...
  #[test]
  fn trusted_proxy_rejects_multiple_forwarded_field_lines() {
    let mut headers = HeaderMap::new();
    headers.insert(header::HOST, HeaderValue::from_static("app.example"));
